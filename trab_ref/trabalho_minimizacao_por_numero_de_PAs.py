...
def funcao_objetivo_quantidade_de_PAs(x: Solucao, dados_do_problema: DadosDoProblema):
    ...
    restricao_numero_maximo_de_PAs = (len(x.solution) - 12)
    restricao_numero_maximo_de_PAs = 0 if restricao_numero_maximo_de_PAs < 0 else restricao_numero_maximo_de_PAs    
    
    
    x.fitness = len(x.solution)   
    x.penalidade = 1000 * restricao_numero_de_clientes_minimo**2 + 400*restricao_numero_maximo_de_PAs**2  #
...
def shake(x: Solucao, k: int, dados: DadosDoProblema):
    ...
    tres_menores_valores = sorted(y.numero_de_clientes_por_ponto_de_acesso.values())[:k]
    chaves_correspondentes = []
    for chave, valor in y.numero_de_clientes_por_ponto_de_acesso.items():
...
def BVNS(solucao_inicial: Solucao, dados: DadosDoProblema) -> [Solucao, Struct]:
    ...
    x = funcao_objetivo_quantidade_de_PAs(solucao_inicial, dados)
    y = copy.deepcopy(x)
...
for x in melhores_solucoes:
    ...
    label = f'Número de PAs {x.fitness} Clientes Atendidos {x.clientes_atendidos_em_porcentagem}'
